...
import type { AppScreen, AppConfig, ChatMessage, Scenario, AirtableSelect, QuizQuestion } from './types'
...
export default function App() {
  const [screen, setScreen] = useState<AppScreen>('member-select')
  ...
  const autoGenerateQuiz = async (manualRecordId: string) => {
    setQuizLoading(true)
    setQuizError('')
    setScreen('quiz-setup') // ローディング画面表示用

    try {
      // 1. マニュアル内容を取得（キャッシュ済みクイズがあればそれを返す）
      ...
      // 3. バックグラウンドでAirtableに保存（失敗しても無視）
      fetch('/api/quiz-save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleRestart = () => {
    ...
    setQuizAnswers([])
    setQuizError('')
    setAirtableSelect({ memberId: '', watchId: '', completeCondition: null, manualRecordId: null })
